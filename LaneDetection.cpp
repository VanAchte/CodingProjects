	...
	while (cap.isOpened()) {
		...
		if (processCount > processFrames) {
			...
			if (numLines > 0) {
				...
				if ((processCount > processFrames) && (processCount % 5 == 0)) {
					numAvgLines++;
					cout << "highestPoint = (" << highestPoint.x << ", " << highestPoint.y << ")" << endl;
					cout << "topAvg = (" << topAvg.x << ", " << topAvg.y << ")" << endl;
					cout << "numAvgLines = " << numAvgLines << endl;
